...
import { seedReviewQueue, getQueueArticle } from 'data/reviewQueue';
 ...
 *
 * Workflow: opened in a new workspace tab from the Knowledge Health
 * page's "Review in Queue" CTA. Presents a left-rail list of articles
 * pending review, and a main pane that shows a side-by-side compare
 * of the current article body (left) against the AI-suggested changes
 * (right).
 *
 * Interaction model:
...
export default class ReviewQueue extends LightningElement {
    ...
    connectedCallback() {
        if (!this._activeId && seedReviewQueue.length) {
            this._switchTo(seedReviewQueue[0].id);
        }
    ...
    get activeArticle() {
        return this._active || seedReviewQueue[0];
    }
    ...
    // ── Page entrance ───────────────────────────────────────────────
    //
    // Counterpart to the Knowledge Health "Review in Queue" exit fade
    // (see `commandCenter.handleReviewInQueue`). Runs once per mount:
    // the page container fades in and lifts up into place so the route
    // hand-off feels like one continuous motion instead of a jump cut.
    // The score-card row gets a small staggered settle on top so the
    // donut + metric column feels alive on arrival.
    _pageEntered = false;
    ...
    handleSaveNext() {
        const idx = seedReviewQueue.findIndex((a) => a.id === this._activeId);
        if (idx < 0) return;
        const current = seedReviewQueue[idx];
        const nextIdx = (idx + 1) % seedReviewQueue.length;
        const next = seedReviewQueue[nextIdx];
    ...
    /**
     * Esc closes the workspace tab (returns to Knowledge Health) —
