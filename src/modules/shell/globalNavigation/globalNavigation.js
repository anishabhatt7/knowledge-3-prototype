...
export default class GlobalNavigation extends LightningElement {
    ...
    handleCloseTab(event) {
        ...
        const page = event.currentTarget.dataset.page;
        window.dispatchEvent(new CustomEvent('workspace:closetab', {
            detail: { page },
        }));
    }
    ...
    handleWaffleMenuKeydown(event) {
        const menu = this.querySelector('.slds-dropdown');
    ...
    _focusWaffle() {
        const waffleIcon = this.querySelector('.slds-context-bar__icon-action lightning-dynamic-icon');
    ...
    _focusFirstMenuItem() {
        const first = this.querySelector('[role="menuitem"]');
        if (first) {
    ...
    _handleDocumentClick(event) {
        const trigger = this.querySelector('[class*="slds-dropdown-trigger"]');
